...
class ShiftableDeviceComponent(DeviceComponent):
    ...
    def update(self):
        ...
        if (not self._shift_pressed):
            ...
            if (self._lock_button != None): #added to update lock state after shift
    ...
    def _lock_value(self, value): #this has been added so that Lock button cannot be used when shifted (conflicts with Bank Select 8)
        if (not self._shift_pressed):
